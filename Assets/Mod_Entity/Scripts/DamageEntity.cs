...
namespace Mod_Entity
{
    ...
    public class DamageEntity : MonoBehaviour
    {
        ...
        private int damageTimes = 1;//对同一对象的伤害次数
        ...
        private void DamageJudge(object aim)
        {
            ...
            if(entity != null)
            {
                if(Tool.isInside(entity.DamageTag,damageTag))//判定是否在判定标签内
                {
                    ...
                    {
                        if(CDok(entity))//判断伤害cd是否冷却
                        {
                            Health health = entity.GetAttribute<Health>();
        ...
        /// <param name="aim">需要判定的对象</param>
        /// <returns></returns>
        private bool CDok(object aim)
        {
            if (timers[aim].timer <= 0) { return true; }
            return false;
        }
        /// <summary>
        /// 使指定判定对象 的计时器重置
